# M30: Harness Orchestration Expansion
...
|---|-------------|--------|--------|--------|
| 1 | Case intake workflow state machine (`matchFromText`) | `feat/m30-intake-state-machine` | ⬜ Planned | 12h |
| 2 | Routing workflow state machine pilot | `feat/m30-routing-state-machine` | ⬜ Planned | 12h |
| 3 | JDBC `AgentPlanArtefactStore` + chat export `plan` schema | `feat/m30-plan-persistence` | ⬜ Planned | 8h |
| 4 | `EvaluationServiceIT` + raise eval baseline from first green run | `feat/m30-eval-it` | ⬜ Planned | 8h |
| 5 | A2A tool-scope enforcement via `ChatToolContextHolder` on bridge | `feat/m30-a2a-tool-scope` | ⬜ Planned | 4h |
| 6 | Grafana harness metrics panel doc + admin cross-link | `feat/m30-harness-ops` | ⬜ Planned | 3h |
| 7 | *(Optional)* Human checkpoint API (`NEEDS_HUMAN` state) | `feat/m30-human-checkpoint` | ⬜ Deferred | 12h |
| 8 | Modulith events: analysis → match handoff | `feat/m30-modulith-handoff` | ⬜ Deferred | 10h |
