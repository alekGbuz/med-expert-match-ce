# M29: Harness Engineering Improvements
...
|---|-------------|--------|--------|--------|
| 1 | `AgentResponseVerifier` — structured post-tool validation | `feat/harness-response-verifier` | ⬜ Planned | 8h |
| 2 | `CaseContextBundleService` + tool for workflows/chat | `feat/harness-context-bundle` | ⬜ Planned | 12h |
| 3 | `MedicalAgentCriticService` — policy pass (disclaimer, PHI, grounding) | `feat/harness-critic-pass` | ⬜ Planned | 8h |
| 4 | Planner artefact model + persistence on chat/workflow runs | `feat/harness-planner-artefact` | ⬜ Planned | 10h |
| 5 | `ChatAgentToolScope` — enforce tools per `ChatAgentProfile` | `feat/harness-tool-scope` | ⬜ Planned | 6h |
| 6 | Eval CI gate + baseline report (`medical-eval-v1`) | `feat/harness-eval-ci` | ⬜ Planned | 6h |
| 7 | Workflow iteration limits + `HarnessFailureReason` codes | `feat/harness-failure-codes` | ⬜ Planned | 6h |
| 8 | Agent card `allowedTools` + server enforcement (A2A/chat) | `feat/harness-agent-card-tools` | ⬜ Planned | 6h |
| 9 | Doctor-match workflow state machine (pilot) | `feat/harness-match-state-machine` | ⬜ Planned | 16h |
| 10 | Harness telemetry metrics (Micrometer, no PHI) | `feat/harness-metrics` | ⬜ Planned | 4h |
| 11 | *(Optional)* Human checkpoint API for workflows | `feat/harness-human-checkpoint` | ⬜ Deferred | 12h |
| 12 | Dev harness docs (Ralph Loop in `testing` skill + backlog template) | `feat/harness-dev-docs` | ⬜ Planned | 3h |
...
**Next milestone after M29:** M30 — extend state machine to case intake + routing workflows; Modulith event handoffs (proposition Phase C2).
